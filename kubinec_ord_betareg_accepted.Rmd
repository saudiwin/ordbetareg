...
# biblatexoptions: [backend=biber,giveninits,pagetracker=true,autocite=inline,alldates=comp,labeldateparts=true,
#   citetracker=true,useeditor=true,usetranslator=true,
...
# save intermediate files for submission
...
require(posterior)
require(brms)
# for custom fonts -- need to have fonts registered (Verdana)
require(extrafont)
require(emojifont)
...
# otherwise will load them from disk
run_model <- F

# whether to do all the marginal effects calculation
...
all_sim %>%
  ...
  mutate(model=factor(model, 
                      levels=c("Ordered Beta Regression",'ZOIB',"OLS","Fractional Logit"))) %>% 
  ...
  labs(caption=stringr::str_wrap("Summary smooth calculated via generalized additive model. M Errors  and S errors are magnitude of bias and incorrect sign of the estimated marginal effect. Variance refers to estimated posterior variance (uncertainty) of the marginal effect.",width=70)) +
  guides(color=guide_legend(title=""),
         linetype=guide_legend(title="")) +
  theme_tufte() +
  theme(text=element_text(family="Verdana"),plot.caption = element_text(size=7),
        axis.text.x=element_text(size=8))

ggsave("figures/figure_1.pdf",height=4,width=6,dpi=600)
...
ggsave("figures/figure_2.pdf",height=4,width=6,dpi=600)
...
tibble(therm=model_data$therm*100) %>% 
  ...
  geom_vline(xintercept=mean(plogis(fit_pew$draws("cutpoints[2]")))*100,
             linetype=2) +
  xlab("") +
  labs(caption=paste0("Figure shows the distribution of ",sum(!is.na(model_data$therm))," country-year suffrage index values.\nThe two vertical lines indicate the estimated cut points from an ordered beta\nregression model where responses have a 0.5 probability of being\nconsidered degenerate (i.e. close to 0 or 1).")) +
  theme(text=element_text(family="Verdana"),plot.caption = element_text(size=9))

#ggsave("college_prof.png",width=5,height=3,units="in",scale=1.1)

ggsave("figures/figure_3.pdf",height=4,width=6,dpi=600)
...
all_vars_ord <- lapply(just_vars,function(c) {
  ...
  saveRDS(all_vars_ord,"data/all_vars_ord.rds")
    
  } else {
    
    all_vars_ord <- readRDS("data/all_vars_ord.rds")
...
saveRDS(all_vars_zoib, "data/all_vars_zoib.rds")
    
  } else {
    
    all_vars_zoib <- readRDS("data/all_vars_zoib.rds")
  }
...
all_vars_Beta_trans <- lapply(just_vars+1,function(c,X_beta=NULL) {
  ...
  saveRDS(all_vars_Beta_trans, "data/all_vars_Beta_trans.rds")
    
  } else {
    
    all_vars_Beta_trans <- readRDS("data/all_vars_Beta_trans.rds")
  }
...
frac_int,mat_data) %>% bind_rows
  ...
  saveRDS(all_vars_frac,"data/all_vars_frac.rds")
     
   } else {
     
     all_vars_frac <- readRDS("data/all_vars_frac.rds")
...
loo_p <- tibble(Model=c("Ordered\nBeta","ZOIB",
  ...
  ggsave("figures/figure_4.pdf",height=4,width=6,dpi=600)
...
ggsave("figures/figure_5.pdf",height=4,width=6,dpi=600)
```
...
recode_marg <- bind_rows(list(OLS=gather(as_tibble(out_lm),key="variable",value="marg"),
  ...
  xlab("") +
    labs(caption="Uncertainty  intervals are the 5% to 95% empirical posterior quantiles.") +
  coord_flip()  +
  theme(text=element_text(family="Verdana"),
        axis.text.y = element_text(size=7),
        plot.caption = element_text(size=8))
  
  ggsave("figures/figure_6.pdf",height=7,width=6,dpi=600)
...
compare_prob <-  bind_rows(list(`ZOIB`=zoib_all_prob,
  ...
  saveRDS(compare_prob,"data/compare_prob.rds")
} else {
  
  compare_prob <- readRDS("data/compare_prob.rds")
...
compare_prob %>% 
  ...
  xlab("") +
  coord_flip() +
  labs(caption=stringr::str_wrap("Plot compares estimated marginal effects for degenerate and continuous responses separately  for ordered beta regression and ZOIB models.",
                                 width=70)) +
  theme(text=element_text(family="Verdana"),
        plot.caption = element_text(size=7))

ggsave("figures/figure_7.pdf",height=7,width=6,dpi=600,device=grDevices::cairo_pdf)
...
I thank Adam Slez, Stijn Masschelein, Staffan Betner, Jonas Kristoffer Lindeløv, Matti Vuorre and anonymous reviewers for helpful comments. I thank New York University Abu Dhabi for computational support.
...
All data and code necessary to reproduce this paper's results can be found at the Dataverse repository [@kubinecReplicationDataOrdered2022] at <https://doi.org/10.7910/DVN/5XYO7O>.

# Figure Legend

Figure \@ref(fig:lookurt). Bias in Estimates As a Function of Sample Size.

Figure 2. Total Variance of ZOIB Marginal Effects as a Function of the Number and Correlation of Covariates.

Figure \@ref(fig:loaddata). Suffrage Index from Aidt and Jensen (2012) with Estimated Cutpoint Locations.

Figure \@ref(fig:modcompare). Comparison of Model Diagnostics for Replication of Aidt and Jensen (2012).

Figure \@ref(fig:postpred). Comparison of Sample to Posterior Empirical Cumulative Distribution for Models.

Figure \@ref(fig:combinecoef). Estimated Marginal Effects of Aidt and Jensen (2012) Covariates on Suffrage Index (0 - 100).

Figure \@ref(fig:zoibcoef). Comparison of Intermediate Probabilities for Ordered Beta Regression and ZOIB.
