...
all_sim %>%
  ...
  filter(!is.na(model)) %>% 
  filter(model %in% c("OLS","Ordinal Beta Regression",'ZOIB')) %>% 
  ...
  labs(caption=stringr::str_wrap("Summary smooth calculated via generalized additive model. M Errors  and S errors are magnitude of bias and incorrect sign of the estimated marginal effect. Variance refers to estimated posterior variance (uncertainty) of the marginal effect.",length=80)) +
