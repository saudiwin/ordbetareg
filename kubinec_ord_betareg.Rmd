...
```{r fitordreg}
  ...
  if(run_model) {
    fit_pew_infl <- sampling(ord_beta_mod, 
                        seed=random_seed,
  ...
  if(run_model) {
    ...
    fit_saveRDS(fit_pew_infl,"data/fit_pew_infl.rds")
  } else {
    fit_pew_infl <- readRDS("data/fit_pew_infl.rds")
  }
...
all_vars_ord_infl <- lapply(1:ncol(mat_data),function(c) {
  ...
  if(all(mat_data[!is.na(mat_data[,c]),c] %in% c(0,1))) {
    ...
    pred_data_high_miss <- mat_data_miss
    
    pred_data_low_miss <- mat_data_miss
    
    if(colnames(mat_data)[c] %in% colnames(mat_data_miss)) {
      
      k <- which(colnames(mat_data_miss)==c)
      
      pred_data_high_miss[,k] <- 0

      pred_data_low_miss[,k] <- 1
    } 
    ...
    if(colnames(mat_data)[c] %in% colnames(mat_data_miss)) {
      ...
      pred_data_high_miss[,k] <- pred_data_high_miss[,k] + setstep(pred_data_high[,c])

      pred_data_low_miss[,k] <- pred_data_low_miss[,k] - setstep(pred_data_low[,c])
    }
  ...
  margin_ord <- sapply(1:nrow(X_beta_ord), function(i,this_col) {
    y0 <- predict_ordbeta(cutpoints=cutpoints_est[i,],
                          X=pred_data_low,
                          X_miss = pred_data_low_miss,
                          X_beta_miss=X_beta_ord_miss[i,],
                          X_beta=X_beta_ord_infl[i,])
    
    y1 <- predict_ordbeta(cutpoints=cutpoints_est[i,],
                          X=pred_data_high,
                          X_miss = pred_data_high_miss,
                          X_beta_miss=X_beta_ord_miss[i,],
                          X_beta=X_beta_ord_infl[i,])
...
# these constitute marginal effects for OLS
out_lm <- as.matrix(pew_fit_lm)[,-1]
...
num_zeros_ols <- apply(yrep_ols,1,function(c) sum(c<=0))

num_ones_ols <- apply(yrep_ols,1,function(c) sum(c>=1))

num_zeros_ord <- apply(yrep_ord,1,function(c) sum(c==0))

num_ones_ord <- apply(yrep_ord,1,function(c) sum(c==1))

num_zeros_zoib <- apply(yrep_zoib,1,function(c) sum(c==0))

num_ones_zoib <- apply(yrep_zoib,1,function(c) sum(c==1))
...
lm_dens <- ppc_dens_overlay(as.numeric(model_data$therm)[model_data$therm>0 & model_data$therm<1],yrep_ols[sample_rows,model_data$therm>0 & model_data$therm<1])
ord_dens <- ppc_dens_overlay(c(to_bl$outcome_degen,to_bl$outcome_prop),yrep_ord[sample_rows,])
ord_dens_infl <- ppc_dens_overlay(as.numeric(to_bl_infl$outcome_prop),yrep_ord_infl[sample_rows,(to_bl_infl$N_degen+1):ncol(yrep_ord_infl)])
zoib_dens <- ppc_dens_overlay(as.numeric(model_data$therm),yrep_zoib[sample_rows,])
beta_trans_dens <- ppc_dens_overlay(as.numeric(model_data$therm),yrep_beta_trans[sample_rows,])
# need to make 
...
recode_marg <- bind_rows(list(OLS=gather(as_tibble(out_lm),key="variable",value="marg"),
  ...
  filter(!(variable %in% c("R2","sigma","log-fit_ratio"))) %>% 
...
```{r sdcoef,include=F,fig.cap="Ratio of 5% - 95% HPD Intervals from Ordered Regression to Other Models"}
  ...
  #           low=quantile(marg,.05)) %>% 
  # group_by(variable) %>% 
  # mutate(inflation=(high-low)/(high[model=="Ordered\nBeta"]-low[model=="Ordered\nBeta"])) %>% 
  summarize(sd_est=mean(abs(marg))/((quantile(marg,.95)-quantile(marg,.05)))) %>% 
  # group_by(model) %>% 
  # summarize(mean_infl=mean(inflation)) %>% 
  ggplot(aes(y=sd_est,x=reorder(model,sd_est))) +
...
loo_ord_cont <- loo(ord_log[,,((nrow(model_data_degen)+1):nrow(model_data))],
                    r_eff=relative_eff(exp(ord_log[,
                                                   ,(nrow(model_data_degen)+1):nrow(model_data)])),
                    cores=2)

loo_zoib_cont <- loo(zoib_log[,,!(model_data$therm %in% c(0,1))],
                    r_eff=relative_eff(exp(zoib_log[,,!(model_data$therm %in% c(0,1))])),
                    cores=2)

loo_lm_disc <- loo(lm_log[,model_data$therm %in% c(0,1)],
                    cores=2)

loo_ord_cont <- loo(ord_log[,,1:nrow(model_data_degen)],
                    r_eff=relative_eff(exp(ord_log[,,1:nrow(model_data_degen)])),
                    cores=2)
...
loo_lm_cont <- loo(lm_log[,!(model_data$therm %in% c(0,1))],
                    cores=2)

loo_compare(loo_ord_disc,loo_zoib_disc,loo_lm_disc)
...
model_data %>% select(Income=income,Education=education,everything()) %>% 
  ...
  summarize(`Mean Thermometer`=round(mean(therm)*100,1),
            ...
            `Mean Degenerate Thermometer`=round(mean(extreme)*100,1)) %>% 
  ...
  mutate(`Mean Degenerate Thermometer`=cell_spec(`Mean Degenerate Thermometer`,
                                                 bold=ifelse(Value %in% "Less than high school",
                                                             "Less than $10,000"))) %>% 
  select(-Variable) %>% 
...
zoib_all <- lapply(1:ncol(mat_data),function(c,coef_a,coef_m,coef_g,alpha) {
